...
class Auth
{
    ...
    protected function getAuthList($uid, $type)
    {
        ...
        if (Session::has('_auth_list_' . $uid . $t)) {
        ...
        //读取用户组所有权限规则
        $rules = Db::table('menu')->where($map)->field('controller,name,action')->select();
        //循环规则，判断结果。
        $authList = []; //
        foreach ($rules as $rule) {
            $authList[] = strtolower("index/".$rule['controller'].'/'.$rule['action']);
        }
