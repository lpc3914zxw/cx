...
class Orders extends Base {
      ...
      //校验是否已实名
        ...
        if($advancedInfo['type'] != 3) {
            ...
            if($preAdvanced) {  // 查看上一个进阶的课程是否学完
                // 上一进阶读取完的数量
                $preLearnedCount = $order_model->where(['advanced_id'=>$preAdvanced['id'],'uid'=>$this->uid,'status'=>1])->count();
                if($preAdvanced['chapter_count'] != $preLearnedCount) {
                    //return returnjson(1001,'','请先学完上一个课程');
                }
            }else{
                $nowLearnedCount = $order_model->where(['advanced_id'=>$advanced_id,'uid'=>$this->uid,'status'=>['in',[1,2]]])->count();
    ...
    public function submitPayData($order_id = '',$pay_type = 1) {
        ...
        $order_model = new \app\wxapp\model\Orders();
        $orderInfo = $order_model->field('value,status,advanced_id')->where('order_id',$order_id)->find();
        ...
        $advanced_model = new Advanced();
        $course_model = new Course();
        $advancedInfo = $advanced_model->field('type,studying_num,reward,value,deadline,pay_type,learn_power')->where(['id'=>$orderInfo['advanced_id']])->find();
        $nowLearnedCount = $order_model->where(['advanced_id'=>$orderInfo['advanced_id'],'uid'=>$this->uid,'status'=>['in',[1,2]]])->count();
                if($nowLearnedCount >= $advancedInfo['studying_num']) {
                    return returnjson(1001,'','该进阶最多同时学习'.$advancedInfo['studying_num'].'课程');
                }
        Db::startTrans();
   ...
   */
    public function unlockCourse($course_id = 0,$uid = 0) {
        ...
        $orderCount = $order_model->where(['course_id'=>$course_id,'uid'=>$this->uid,'status'=>['in',[1,2]]])->count();
          	...
          	if($type == 2){//微信支付
          	    ...
          	    if($is_face == 0){
                    if($res['alipay_trade_app_pay_response']['code'] == '10000'){
                            ...
                            $orderres = Db::name('order')->where(['out_trade_no'=>$res['alipay_trade_app_pay_response']['out_trade_no']])->find();
                    ...
                    if($res['alipay_trade_app_pay_response']['code'] == '10000'){
                            ...
                            $res = Db::name('face_order')->where(['out_trade_no'=>$res['alipay_trade_app_pay_response']['out_trade_no']])->find();
                            //$res = Db::name('face_order')->where('out_trade_no',$out_trade_no)->find();
                            //return returnjson(1000,'','支付成功');
                            if($res['status']==1){
                                return returnjson(1000,'','支付成功');
