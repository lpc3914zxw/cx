...
class Course extends Base{
    ...
    public function index() {
        ...
        $tjCourseData = $course_model->field('id,teacher_id,name')->where('is_tj',1)->where(['is_delete'=>0,'is_shelves'=>1])->limit(3)->select();
        if($tjCourseData) {
        ...
        // 每次一课
        $dayCourse = $course_model->field('id,advanced_id,name,abstract,samll_imgurl,people_num')->where('is_daycourse',1)->where(['is_delete'=>0,'is_shelves'=>1])->find();
        ...
        foreach ($advancedList as $k=>$val) {
            ...
            $courseList = $course_model->field('id,name,abstract,samll_imgurl,people_num')
                        ->where('advanced_id',$val['id'])->where(['is_delete'=>0,'is_shelves'=>1])->order('sort')->limit(4)->select();
