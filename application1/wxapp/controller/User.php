...
use app\wxapp\model\LongUserLike;
...
class User extends Base{
    ...
    public function myInfo() {
        ...
        $userInfo = $user->where('id',$this->uid)->field('id,name,headimg,student_no,level,start_level,signature,score,wechat,tel,is_auth,wximg,province,city,area,gender,year,occupation,industry,education,street,provinceid,cityid,areaid,streetid')->find();
        ...
        if($userInfo){   //is_auth 0未认证 1已提交审核 2已认证 3认证驳回
            ...
            if(!empty($userInfo['industry'])){
                $userInfo['industryname'] = Db::name('industry')->where('id',$userInfo['industry'])->value('name');
            }else{
                $userInfo['industryname'] = '';
            }
            
            if(!empty($userInfo['occupation'])){
                $userInfo['occupationname'] = Db::name('occupation')->where('id',$userInfo['occupation'])->value('name');
            }else{
    ...
    public function medal(){
        ...
        foreach($startInfo as $skey=>$svalue){
            ...
            $startInfo[$skey]['getState'] = false;
            if($userInfo['start_level'] == $svalue['value']){
                $startInfo[$skey]['getState'] = true;
        ...
        $start_level_content = Db::name('system')->where('id',1)->value('start_level_content');
        $data['userinfo'] = $userInfo;
        $data['startinfo'] = $startInfo;
        $data['start_level_content'] = $start_level_content;
    ...
    public function userMore(){
        ...
        if(empty($data)){
            return returnjson(1001,'','没有提交任何数据');
        }
        $user_model = new \app\wxapp\model\User();
        
        $user_model->where('id', $this->uid)->update($data);
    ...
    */
    ...
    public function inviteClass() {
      ...
      $res1 = $common_model->isMaxOrMin($this->uid,1);
        ...
        $data = ['level'=>$mylevel,'childCount'=>$res['direct'],'sameGrade'=>$sameGrade,'otherGrade'=>$otherGrade,'sumContribution'=>$myinfo['dedication_value'],'comContribution'=>$res['minshequnD'],'alumnusSum'=>$res['total_num'],'authenticatedNumber'=>$res1['total_num'],'puserInfo'=>$puserInfo,'invite_class_content'=>$invite_class_content];
        return returnjson(1000,$data,'获取成功');
    }
    ...
    public function myChildren() {
        ...
        if(!is_null(input('status'))){
            ...
            $userList = $user_model->field('id,name,headimg,score,is_auth,level,regetime,class_dedication,learning_power,dedication_value')->where('pid',$this->uid)->where($where)->limit($limit)->order('dedication_value','desc')->select();
        }else{
            $status = 0;
            $userList = $user_model->field('id,name,headimg,score,is_auth,level,regetime,class_dedication,learning_power,dedication_value')->where('pid',$this->uid)->limit($limit)->order('dedication_value','desc')->select();
        }
      ...
      $common_model = new common();
            ...
            foreach ($userList as $k=>$val) {
                if($val['is_auth'] == 0) {
              	...
              	$userList[$k]['class_dedication'] = round($val['class_dedication'],2);
              	$userList[$k]['regetime'] = date('Y.m.d H:i',$val['regetime']);
              	$userList[$k]['noAuthNum'] = $common_model->getShequnNum($val['id'],1);
              $userList[$k]['authNum'] = $common_model->getShequnNum($val['id'],0);
              	//$userList[$k]['noAuthNum'] = 0;
              //$userList[$k]['authNum'] = 0;
             // $res = $common_model->isMaxOrMin($val['id'],1,0);
              $userList[$k]['score'] = $userList[$k]['noAuthNum'];
       ...
       // $fontsize1 = 30;
        ...
        if($tempType == 0) {
            ...
            if ($logo !== FALSE) {
                ...
                $logo_qr_width = 90; //头像的宽度
                $logo_qr_height = 90; //头像高度
                $from_width = 30; //距离左边距的宽度
                $from_height = 623; //距离左边距的高度
                //重新组合图片并调整大小
                imagecopyresampled($QR, $logo, $from_width, $from_height, 0, 0, $logo_qr_width,$logo_qr_height, $logo_width, $logo_height);
                //$lasefilename = 'qrcode/'.$uid.rand(100,999).'.png';//原图
                $lasefilename = 'qrcode/'.$uid.rand(50,500).'.png';//原图
                imagepng($QR, $lasefilename);
            }
          ...
          //$height = 1260;
            foreach ($textArray as $val) {
             ...
             // $height+= 60;
                ...
                imagettftext($image,$fontsize,0,150,$height,$bluecolor,$fontfile,$val);
            }
            //第一个是画布资源 2 文字大小 3 旋转角度  4 x坐标  5  纵坐标
    ...
    public function courseList(){
        ...
        if(empty(input('status'))||input('status') ==0){
            $owhere['status'] = array('neq',0);
        }else{
            $owhere['status'] = input('status');
        ...
        $start = ($page - 1) * 16;
        $limit = $start . ',' . 16;
        $orderInfo = $order_model->where(['uid'=>$this->uid])->where($owhere)->field('id,course_id,paytime,status')->limit($limit)->order('status desc,paytime desc')->select();
        $list = array();
        ...
        if(!empty($orderInfo)){
            ...
            foreach($orderInfo as $okey=>$oval){
                $cour = $course_model->where(['id'=>$oval['course_id']])->field('id,name,chapter_count,abstract,imgurl,advanced_id,teacher_id,deadline')->find();
                if(empty($cour['imgurl'])){
                    $cour['imgurl'] = '';
                }
                $learnedCount = $learnLog_model->where(['course_id'=>$oval['course_id'],'uid'=>$this->uid])->count();
                ...
                $isStudy = Db::name('CourseLearnLog')->alias('cll')->join('task_result','cll.id=task_result.cllid','left')->where(['task_result.uid'=>$this->uid,'cll.course_id'=>$oval['course_id'],'task_result.addtime'=>['between',[$beginToday,$endToday]]])->find();
                ...
                $paytime = $oval['paytime'];
                $days = intval((time() - $paytime) / 86400);
                $yudays = ($advanced['deadline'] - $days) < 0 ? 0 : $advanced['deadline'] - $days;
                if($advanced['deadline'] - $days<0){
                    if($oval['status']==1){
                        Db::name('order')->where('id',$oval['id'])->update(['status'=>2]);
                        $oval['status'] =2;
                ...
                $list[$okey] = $cour;
                if($oval['status'] == 1) {
                     $list[$okey]['status'] = '学习中';
                }else if($oval['status'] == 2){
                     $list[$okey]['status'] = '已过期';
                     $advanced['learn_power'] = 0;
                }else if($oval['status'] == 3){
                     $list[$okey]['status'] = '已完成';
                     $advanced['learn_power'] = 0;
                }
                
                $list[$okey]['days'] = $yudays>=0 ? $yudays.'天':'已过期';
                $list[$okey]['learnPer'] = intval($learnPer * 100);
                $list[$okey]['chapter_count'] = $chapter_count;
                $list[$okey]['deadline'] = $advanced['deadline'];
                $list[$okey]['learn_power'] = $advanced['learn_power'];
                $list[$okey]['isStudy'] = $isStudy;
    ...
    public function longRankSet() {
        ...
        $rankimg = $userInfo['headimg'];
        $user_model = new \app\wxapp\model\User();
        $userData = $user_model->getLongRankSetInfo($this->uid,$type);
     	 $data = $user_model->getLongRankList($type,1,$this->uid);
     	 $longcontent = Db::name('system')->where('id',1)->value('longcontent');
        $userData['rankimg'] = $data[0]['headimg'];
        $starttime = '2020.08.01';
        $endtime = '2020.10.01';
        $userData['starttime'] = $starttime;
        $userData['endtime'] = $endtime;
        $userData['longcontent'] = $longcontent;
        return returnjson(1000,$userData,'获取成功');
    }
    ...
    public function longRankList() {
        ...
        $limit = $start.','.$this->num;
        $user_model = new \app\wxapp\model\User();
        $data = $user_model->getLongRankList($type,$limit,$this->uid);
        return returnjson(1000,$data,'获取成功');
    }
    ...
    public function longUserLike() {
        ...
        if($status == 0) {  // 取消点赞
            ...
            if(false === $user_model->where('id',$likeUid)->setDec('long_like_num')) {
                Db::rollback();
                return returnjson(1001,'','取消错误');
            }
            Db::commit();
        ...
        if(false === $user_model->where('id',$likeUid)->setInc('long_like_num',1)) {
            Db::rollback();
            return returnjson(1001,'','点赞错误');
        }
        Db::commit();
   ...
   */
    ...
    public function myFootprintLog() {
        ...
        $startPage = ($page - 1) * 5;
        $dedicationLog = new DedicationLog();
        $sql = "select FROM_UNIXTIME(addtime, '%Y-%m-%d') as datetime from dedication_log  where type!=17 and type!=18 and type!=19 and type!=20 and type!=21 and uid = ".$this->uid." group by datetime  order by datetime desc limit ".$startPage.",".'5';
        //echo $sql;exit;
        $list = Db::query($sql);
        if($list) {
            foreach ($list as $k=>$val) {
                ...
                $end = $start + 86399;
                $daylist = $dedicationLog->where(['uid'=>$this->uid,'addtime'=>['between',[$start,$end]]])->where('type','neq',17)->where('type','neq',18)->where('type','neq',19)->where('type','neq',20)->where('type','neq',21)->order('addtime desc')->select();
