...
class Section extends Base
{
    ...
    */
    ...
    public function getSectionDetail($s_id = 0) {
        ...
        $paytime = $order_model->where(['uid'=>$this->uid,'course_id'=>$courseInfo['id'],'status'=>['in',[1,2,3]]])->value('paytime');
    ...
    public function submitTask ($task_id = 0,$optionids = '') {
     ...
     // if($token!='606b934c883f065debbbd54c338f0767'){
      ...
      $ishas = $learnLog->where(['section_id'=>$section_id,'uid'=>$this->uid])->find();
        ...
        // 判断是否已经解锁  // 查看
        $thisSection = $section_model->field('id,sort')->where(['id'=>$section_id,'c_id'=>$c_id])->order('sort asc')->find();
        $preSection = $section_model->field('id')->where(['sort'=>['lt',$thisSection['sort']],'c_id'=>$c_id])->where('is_delete',0)->order('sort desc')->find();
        ...
        $owhere['id'] = array('in',$optionids);
        //$optionList = $option_model->where(['task_id'=>$task_id,'id'=>['in',[$optionids]]])->select();
        $optionList = Db::name('task_option')->where(['task_id'=>$task_id])->where($owhere)->select();
     ...
     // }
        ...
        // 判断是否已过期 过期则不给予学分
        $paytime = $order_model->where(['uid'=>$this->uid,'course_id'=>$c_id,'status'=>['in',[1,2,3]]])->value('paytime');
        //$days = intval((date('d',time()) - date('d',$paytime)) / 86400); // 学习该课程的天数
        $days = intval((time() - $paytime) / 86400); // 学习该课程的天数
        // 提交答案
        $data = ['uid'=>$this->uid,'task_id'=>$task_id,'addtime'=>time(),'cllid'=>$ishas['id']];
        Db::startTrans();
...
//                }
        ...
        $this->unlockCourse($c_id,$this->uid,$section_id);
    ...
    * 进入已购课程列表页面开始解锁操作
    */
    public function unlockCourse($course_id = 0,$uid = 0,$section_id=0) {
      	//if(empty($this->uid)){
        //	$this->uid = $uid;
       // }
      	$user_model = new \app\wxapp\model\User();
        ...
        if($orderCount > 0) {
          ...
          //if($section_id!=0){
            //  $ishas = $learnLog->where(['course_id'=>$course_id,'uid'=>$uid,'status'=>0,'section_id'=>$section_id])->find();
         // }else{
              
              $ishas = $learnLog->where(['course_id'=>$course_id,'uid'=>$uid,'status'=>0])->find();
          //}
          
          
          if($ishas){
              //$learnLog->where(['section_id'=>$section_id,'uid'=>$uid])->update(['status'=>1]);
            return;
          }
