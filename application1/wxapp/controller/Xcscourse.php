...
//            }
    ...
    public function getCourseInfo($course_id = 0) {
        ...
        $advanced_model = new Advanced();
        if($order_model->where(['uid'=>$this->uid,'course_id'=>$course_id,'status'=>['in',[1,2,3]]])->find()) {
            ...
            $deadline = $advanced_model->where('id',$courseInfo['advanced_id'])->value('deadline');
            ...
            $paytime = $order_model->where(['uid'=>$this->uid,'course_id'=>$course_id,'status'=>['in',[1,2]]])->value('paytime');
            $days = intval((time() - $paytime) / 86400);
            
            
            $courseInfo['days'] = ($deadline - $days) < 0 ? 0 : $deadline - $days;
...
//        }
    ...
    public function buyedCourse($course_id = 0,$page = 1) {
        ...
        $orderInfo = $order_model->where(['course_id'=>$course_id,'uid'=>$this->uid,'status'=>['in',[1,2,3]]])->find();
