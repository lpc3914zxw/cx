...
//file_put_contents($documentRoot.'/log_0807__2.txt',print_r('邀请数量：'.($userInfo['invate_num']+ 1).'升级数量:'.$startLevelInfo['invite_people'].'荣誉值：'.($userInfo['dedication_value'] + 1).'升级数量：'.$startLevelInfo['contribution'].'拥有小沈群贡献值'.($smallSqCount + 1).'升级小社群'.$startLevelInfo['small_sq'].'最大等级'.$maxStartLevel.'下一级：'.$nextStartLevel.'是否是真：'.$true,true),FILE_APPEND);
    ...
    public function userChangeStartLevel($uid = 0) {
      ...
      $true = 0;
       if($nextStartLevel==2){
            $punm = $user_model->where('is_auth',1)->where('pid',$uid)->where('start_level','egt',1)->count();
         	if($punm>=2){
            	$true = 1;
            }
      ...
      if($uid == 566){
        	//$documentRoot = $_SERVER['DOCUMENT_ROOT'];
//file_put_contents($documentRoot.'/log_0825__1.txt',print_r('邀请数量：'.($userInfo['invate_num']+ 1).'升级数量:'.$startLevelInfo['invite_people'].'荣誉值：'.($userInfo['dedication_value'] + 1).'升级数量：'.$startLevelInfo['contribution'].'拥有小沈群贡献值'.($smallSqCount + 1).'升级小社群'.$startLevelInfo['small_sq'].'最大等级'.$maxStartLevel.'下一级：'.$nextStartLevel.'是否是真：'.$true.'数量'.$punm,true),FILE_APPEND);
        }
    ...
    public function sendAdvanceCourse($uid = 0,$advanced_id = 0,$type = 0,$cid = 0) {
        ...
        $advanced_model = new Advanced();
            ...
            $cidswhere = array();
            $my_course_ids = Db::name('order')->where('status','neq',0)->where('uid',$uid)->where('advanced_id',$advanced_id)->field('course_id')->select();
            if(!empty($my_course_ids)){
                $course_ids = '';
                foreach($my_course_ids as $mcival){
                    $course_ids .= $mcival['course_id'].',';
                }
                $course_ids = trim($course_ids,',');
                $cidswhere['id'] = array('not in',$course_ids);
            }
            if($type==3){
                $courseList = $course_model->field('id,imgurl,name,advanced_id')->where('id',$cid)->limit(1)->where('is_delete',0)->where($cidswhere)->select();
                //var_dump($courseList);exit;
                //$advanced_id = $courseList[0]['$courseList'];
            }else{
                $courseList = $course_model->field('id,imgurl,name,advanced_id')->where('advanced_id',$advanced_id)->limit(1)->order('id')->where('is_delete',0)->where($cidswhere)->select();
            }
        ...
        foreach ($courseList as $k=>$val) {
            ...
            if(!$order_model->where(['uid'=>$uid,'course_id'=>$val['id'],'status'=>['in',[1,2,3]]])->find()) {
    ...
    public function unlockCourse($course_id = 0,$uid = 0) {
        ...
        $orderCount = $order_model->where(['course_id'=>$course_id,'uid'=>$uid,'status'=>['in',[1,2]]])->count();

        if($orderCount > 0) {
            $paytime = $order_model->where(['uid'=>$uid,'course_id'=>$course_id,'status'=>['in',[1,2]]])->value('paytime');
...
//获取大神群
    ...
    public function getall($uid,$maxids,$pid){
        $user_model = new User();
        $maxids = Db::name('user') ->where('pid',$uid)->limit(2)->order('dedication_value','desc')->field('id,invate_num,class_dedication')->select();
        $count = count($maxids);
        $maxinvate_num = 0;
        $max_ids = '';
        if($count==2){
            $max_ids = $maxids[0]['id'].','.$maxids[1]['id'];
            $maxinvate_num = $maxids[0]['invate_num']+$maxids[1]['invate_num'];
            $maxlearning_power = $maxids[0]['class_dedication']+$maxids[1]['class_dedication'];
        }else{
            if(isset($maxids[0])){
                $max_ids=$maxids[0]['id'];
                $maxinvate_num = $maxids[0]['invate_num'];
                $maxlearning_power = $maxids[0]['class_dedication'];
            }
            $smallSqCount = 0;
        }
        $smallSqCount = Db::name('user') ->where('pid',$uid)->where('id','not in',$max_ids)->sum('class_dedication');
        $mininvate_num = Db::name('user') ->where('pid',$uid)->where('id','not in',$max_ids)->sum('invate_num');
       $myinfo = Db::name('user')->where('id',$uid)->field('class_dedication,invate_num')->find();
       $total_num = $myinfo['invate_num'];
       $all_power = $myinfo['class_dedication'];
       //$total_num = $mininvate_num + $maxinvate_num;
        return array('mininvate_num'=>$mininvate_num,'minlearning_power'=>$smallSqCount,'maxinvate_num'=>$maxinvate_num,'maxlearning_power'=>$maxlearning_power,'total_num'=>$total_num,'all_power'=>$all_power);
    }
  ...
  //用于定时任务
  	public  function dedicationLog_($uid = 0,$type = 0,$obj_id = '',$content = '',$powerValue=0,$anth = 0) {
        $user_model = new User();
        $collier_model = new Colliers();
        $dedicationLog = new DedicationLog();
            ...
            $collierInfo['contribution'] = $powerValue;
            $sq_type = 0;
  	...
  	function giveOut(){
      $i = 0;
    	$all = Db::name('give_out')->where('status',0)->limit(1000)->select();
    	$beginToday=mktime(0,0,0,date('m'),date('d'),date('Y'));
        $endToday=mktime(0,0,0,date('m'),date('d')+1,date('Y'))-1;
        $anth = Db::name('give_auth')->where(['addtime'=>['between',[$beginToday, $endToday]]])->find();
        $anthId = $anth['id'];
        if(!$anth){
            Db::name('give_auth')->insert(['addtime'=>time()]);
            $anthId = Db::name('give_auth')->getLastInsID();
        }
    	//var_dump($all);exit;
      	foreach($all as $key=>$value){
        	Db::startTrans();
              ...
              $idarray = array();
            	$idarray = explode(',',$ids);
            	if(count($idarray)>5){
            	    Db::name('give_out')->where('id',$value['id'])->update(['status'=>2]);
            	    Db::commit();
            	    continue;
            	}
              	...
              	foreach($idarray as $k=>$v){
                  	...
                  	$i++;
                  	$anth = Db::name('give_auth')->where('id',$anthId)->find();
                    $id_s = explode(',',$anth['ids']);
                    if(in_array($v,$id_s)){
                        $is_auth = 1;
                	...
                	$res = $this->dedicationLog_($v,$value['type'],$value['obj_id'],$value['content'],$value['value'],$is_auth);
                  if($is_auth==0){
                        Db::name('give_auth')->where('id',$anthId)->update(['ids'=>$idss]);
                    }
                  if( false ===$res){
                  	 Db::rollback();
                    return false;
    ...
    //发放族系学习力定时任务_长数
  	function giveOut_long(){
      $i = 0;
    	$all = Db::name('give_out')->where('status',2)->limit(1000)->select();
    	//var_dump($all);exit;
      	foreach($all as $key=>$value){
        	Db::startTrans();
          	$ids = trim($value['ids'],',');
            	...
            	//}
              	foreach($idarray as $k=>$v){
                  	...
                  	$gievdata = [
                  	    ...
                  	    $res = Db::name('give_out_primary')->insert($gievdata);
              	...
              	Db::name('give_out')->where('id',$value['id'])->update(['status'=>3]);
            }
    ...
    //发放族系学习力定时任务_主
  	function giveOut_long_primary(){
      $i = 0;
    	$all = Db::name('give_out_primary')->where('status',0)->where('type',17)->limit(20000)->select();
    	//var_dump($all);exit;
    	$beginToday=mktime(0,0,0,date('m'),date('d'),date('Y'));
        $endToday=mktime(0,0,0,date('m'),date('d')+1,date('Y'))-1;
        $anth = Db::name('give_auth')->where(['addtime'=>['between',[$beginToday, $endToday]]])->find();
        ...
        if(!$anth){
      	...
      	foreach($all as $key=>$value){
            ...
            $anth = Db::name('give_auth')->where('id',$anthId)->find();
            $ids = explode(',',$anth['ids']);
            if(in_array($value['give_uid'],$ids)){
                $is_auth = 1;
                
            }else{
                $is_auth = 0;
                
                $idss = $anth['ids'].','.$value['give_uid'];
            }
            $res = $this->dedicationLog_($value['give_uid'],$value['type'],$value['obj_id'],$value['content'],$value['value'],$is_auth);
            $i++;
            ...
            Db::name('give_out_primary')->where('id',$value['id'])->update(['status'=>1]);
     ...
     * @param int $uid 当前用户id
     */
    public function amend_power_dedication($uid = 0) {
      
        $sum = Db::name('dedicationLog')->where('uid',$uid)->where('type',17)->sum('value');
    ...
    public function reissue_21_de(){
        ...
        $dedicationLog = new DedicationLog();
        $user_model = new User();
        $allorder = Db::name('order')->where('status','neq',0)->where('is_reissue',0)->limit(2000)->field('id,advanced_id,uid,course_id')->select();
        //var_dump($allorder);exit;
        foreach($allorder as $okey=>$oval){
            if($oval['advanced_id']==11){
                $value = 1;
            }elseif($oval['advanced_id']==1){
                $value = 1;
            }elseif($oval['advanced_id']==2){
                $value = 10;
            }elseif($oval['advanced_id']==3){
                $value = 50;
            }elseif($oval['advanced_id']==4){
                $value = 100;
            }elseif($oval['advanced_id']==5){
                $value = 500;
            }elseif($oval['advanced_id']==7){
                $value = 1000;
            }
            Db::startTrans();
            $dedicData = [
            ...
            if(false == $user_model->where('id',$oval['uid'])->setInc('dedication_value',$value)) {
                Db::rollback();
                continue;
            }
            //Db::name('order')->where('id',$oval['id'])->update(['is_reissue'=>1,'reissuetime'=>time()]);
                // 更新用户星际等级
            if(!Db::name('order')->where('id',$oval['id'])->update(['is_reissue'=>1,'reissuetime'=>time()])) {
                Db::rollback();
                continue;
            }
            	Db::commit();
    ...
    //发放课程荣誉值
  	function giveOut_course(){
      $i = 0;
    	$all = Db::name('give_out')->where('status',0)->where('type',21)->limit(1000)->select();
    	//var_dump($all);exit;
      	foreach($all as $key=>$value){
        	Db::startTrans();
          	$ids = trim($value['ids'],',');
              ...
              $idarray = array();
            	...
            	if(count($idarray)>5){
              	...
              	foreach($idarray as $k=>$v){
                  	...
                  	$i++;
                	$res = $this->dedicationLog_($v,$value['type'],$value['obj_id'],$value['content'],$value['value']);
