...
class User extends AdminBase {
    ...
    public function index(){
        ...
        //课程列表
        $courselist = Db::name('course')->where(['is_delete'=>0])->field('id,name,advanced_id')->order('sort')->select();
        foreach($courselist as $cokey=>$coval){
            $ad = Db::name('advanced')->where('id',$coval['advanced_id'])->field('name,type')->find();
            if($ad['type']==2){
                unset($courselist[$cokey]);
                continue;
            }
            $courselist[$cokey]['name'] = $ad['name'].'|--'.$coval['name'];
        }
        ...
        if($this->request->isAjax()){
            ...
            if(isset($params['start_level'])&&$params['start_level']!=''){
            ...
            foreach($list['rows'] as $lkey=>$lval){
                 $common = new Common();
               
                $list['rows'][$lkey]['minShequnD'] = $common->minShequnD($lval['id'],1,1);
  ...
  function is_mobile( $text ) {
    ...
    public function export(){
        ...
        $list = $user_model->get_list_p($where);
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('E1','等级');
        $objPHPExcel->getActiveSheet()->setCellValue('F1','星际等级');
        $objPHPExcel->getActiveSheet()->setCellValue('G1','贡献值');
        $objPHPExcel->getActiveSheet()->setCellValue('H1','学分');
        $objPHPExcel->getActiveSheet()->setCellValue('I1','学习力');
        $objPHPExcel->getActiveSheet()->setCellValue('J1','荣誉值');
        $objPHPExcel->getActiveSheet()->setCellValue('K1','是否认证');
        $objPHPExcel->getActiveSheet()->setCellValue('L1','注册时间');
        $objPHPExcel->getActiveSheet()->setCellValue('M1','上级id');
        foreach ($data as $k=>$val){
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('C'.$i,$val['realname']);
            $objPHPExcel->getActiveSheet()->setCellValue('D'.$i,$val['tel']);
            $objPHPExcel->getActiveSheet()->setCellValue('E'.$i,$val['lname']);
            $objPHPExcel->getActiveSheet()->setCellValue('F'.$i,$val['sname']);
            $objPHPExcel->getActiveSheet()->setCellValue('G'.$i,$val['dedication_value']);
            $objPHPExcel->getActiveSheet()->setCellValue('H'.$i,$val['score']);
            $objPHPExcel->getActiveSheet()->setCellValue('I'.$i,$val['learning_power']);
            $objPHPExcel->getActiveSheet()->setCellValue('J'.$i,$val['honor_value']);
            $objPHPExcel->getActiveSheet()->setCellValue('K'.$i,$is_auth);
            $objPHPExcel->getActiveSheet()->setCellValue('L'.$i,$addtime);
            $objPHPExcel->getActiveSheet()->setCellValue('M'.$i,$val['pid']);
