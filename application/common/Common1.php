...
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('进入学习力内部：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
        if($type == 1) {
		...
		if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('开始族系增加贡献值：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
      ...
      if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('族系增加贡献值完成：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
      ...
      if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('自增学习力：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
        return true;
    }
    ...
    public function userChangeLevel($uid = 0) {
        ...
        $nextLevel = $level_model ->where($lwhere)->order('value')->value('value');
      	if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('进入用户升级：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
        // 当前用户等级升级
...
if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('用户升级完成：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
        $pid = $user_model->where('id',$uid)->value('pid');
        // 上级用户等级升级
        if($pid != 0) {
            ...
            if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('升等级操作：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
           ...
           if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('开始荣誉值：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
            // 荣誉值
      ...
      if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('开始学习力：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
        // 学习力
      ...
      if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('结束学习力：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
        return true;
    }
    ...
    public function changeLevelOpt($uid = 0,$level = 1) {
      ...
      $userIsauth = $user_model->field('is_auth')->where(['id'=>$uid])->find();
      	if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('升等级内部：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
        if($level > 1) {
          ...
          if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('开始赠送免费课程：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
            // 赠送免费课程
          ...
          if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('赠送免费课程结束：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
        }
      if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('开始更新星级等级：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
        // 更新用户星际等级
      ...
      if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('更新星级等级完毕：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
        return true;
    }
    ...
    public function sendAdvanceCourse($uid = 0,$advanced_id = 0) {
      ...
      if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('进入赠送课程内部：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
        ...
        foreach ($courseList as $k=>$val) {
          ...
          if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('生成订单：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
            if(!$order_model->where(['uid'=>$uid,'course_id'=>$val['id'],'status'=>1])->find()) {
                if(!$order_model->insert($data)) {
              	...
              	if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('发放荣誉值：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
              ...
              if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('增加学习人数：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }		
              if($uid == 19136){
              	$missiondata = array(
                	'objid'=>$val['id'],
                ...
                Db::name('mission')->insert($missiondata);
              	...
              	if($uid != 19136){
        	$yuanpeople = $course_model->where('id',$val['id'])->value('people_num');
                if(false === $course_model->where('id',$val['id'])->update('people_num',$yuanpeople+1)) {
                    Db::rollback();
                    return false;
                }
        }		
              if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('开始赠送课程后续操作：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
              ...
              if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('解锁第一节课：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
                // 解锁第一节课
                $this->unlockCourse($val['id'],$uid);
               if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('解锁第一节课结束：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
            }
        }
      if($uid == 19136){
        	$documentRoot = $_SERVER['DOCUMENT_ROOT'];
file_put_contents($documentRoot.'/log_0806__1.txt',print_r('解锁全部结束：'.date('Y-m-d H:i:s',time()),true),FILE_APPEND);
        }
        return true;
    }
...
//获取大神群
    ...
    public function minShequnD($uid,$maxids,$pid){
        ...
        if(!empty($ids)){
            $ids = trim($ids,',');
           // $smallSqCount = $user_model->where('id','in',$ids)->sum('dedication_value');
          	$dedicationLog = new DedicationLog();
           	$smallSqCount = $dedicationLog->where('uid','in',$ids)->where(['type'=>17])->sum('value');
        }
