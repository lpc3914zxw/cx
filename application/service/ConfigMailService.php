...
class ConfigMailService
{
    ...
    public static function ConfigList($params = [])
    {
        ...
        return Db::name('config_mail')->column($field);
    }
    ...
    public static function ConfigSave($params = [])
    {
        ...
        foreach($params as $k=>$v)
        {
            ...
            if(Db::name('config_mail')->where(['only_tag'=>$k])->update(['value'=>$v, 'upd_time'=>time()]))
            {
                $success++;

                // 单条配置缓存删除
                cache(config('mail.cache_config_row_key').$k, null);
            }
        }
        if($success > 0)
        {
    ...
    {
        $key = config('mail.cache_common_my_config_key');
        $data = cache($key);
        if($status == 1 || empty($data))
        {
            // 所有配置
            $data = Db::name('Config_mail')->column('value', 'only_tag');
    ...
    {
        // 缓存key
        $key = config('mail.cache_config_row_key').$only_tag;
        $data = cache($key);

        // 获取内容
        if(empty($data))
        {
            $data = Db::name('config_mail')->where(['only_tag'=>$only_tag])->field('name,value,type,upd_time')->find();
            if(!empty($data))
            {
                // 富文本处理
