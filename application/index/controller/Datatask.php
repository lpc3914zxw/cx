...
class Datatask
{
    ...
    public function index() {
        ...
        for ($beginMonth;$beginMonth+3600;$beginMonth>=$begin){
            echo $beginMonth;
        }
        exit;
        if(Db::name('statistical')->where('type','=',0)->count() == 0){
            //1注册用户
            $user_new = $user_model->count();
            //2今日认证用户
            $user_face=  $user_model->get_face_count();
            // 3本日访问量
            $browse =  $browseRecords_model->count();
            //4今日系统学分产出
            $credit= $creditSource_model->get_score_sum(1, '>');
            //6今日打赏学分产出
            $credit_d=$creditSource_model->get_score_sum(2, '<');
            //7 今日收入
            $money_c =  $order_model->where(['pay_type' => ['in', ("2,5,6")], 'status' => ['neq', 1]])->sum('value');
            // 8今日学分收入
            $credit_c =$order_model->where(['pay_type' => ['in', ("1")], 'status' => ['neq', 1]])->sum('value');
            $honor=$honorlog_model->sum('value');
            $learningpower=$learningpowerlog_model->sum('value');
            $dedication=  $dedicationLog_model->sum('value');
