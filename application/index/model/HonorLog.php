...
class HonorLog extends Model {
    ...
    */
    public function getApiHornorList($where = [],$limit = '') {
        ...
        $reciveDedica = $this::where($where)->where(['type'=>['in',("1,2,3,4,5")]])->sum('value');
        $payDedica = $this::where($where)->where(['type'=>6])->sum('value');
    ...
    public static function getTypeName($type = 0)
    {
        $arr =  [
            ...
            '4' => '分享每日金句1次', '5' => '转发文章一次','6'=>'兑换TLT','7'=>'平台充值','8'=>'平台扣除','9'=>'赠送课程'
        ];
        return $arr[$type];
